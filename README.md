...
使用java（spring boot+vert.x）开发的跨平台的内网穿透工具。

支持功能如下：
1. 端口映射穿透：HTTP、HTTPS、TCP、UDP
2. 正向代理穿透：HTTP代理、HTTPS代理、STCP(sock4/SOCKS5 TCP)，**HTTP、HTTPS都可以穿透为HTTPS**。
3. P2P穿透：UDP打洞实现P2P穿透，如果打洞失败，仍可使用端口转发访问。
4. 统一认证：**所有穿透服务都需要通过用户名和密码认证**，服务端设置默认用户名、密码，**客户端可自定义用户名、密码**。
5. 稳定安全：断线重连功能、开启ssl功能。
6. 配置热加载：客户端穿透配置热加载，通过web页面修改配置，配置可存文件或redis。
7. 注册留痕：服务端持久化客户端穿透注册信息到磁盘配置文件。
...
1. 软件技术架构：
   ...
   Spring Boot 2.7.14：运行控制、配置管理 

   Vert.x 4.5.3：服务管理、服务代理、服务中转

   vue3+element-plus：web端管理配置信息
2. 功能实现图解：
...
5. P2P内网穿透：点对点内网穿透，client支持在用户端部署，服务端增加协助UDP打洞功能，打洞成功后，流量不再经过服务器中转。
   打洞功能使用流程：
   1. 客户端穿透配置启用P2P打洞支持，与服务端建立连接。
   2. 服务端初始化打洞服务，穿透端口还是服务器中转访问，注册端口协助UDP打洞。
   3. 用户电脑上运行客户端，配置穿透端口和本地访问端口，与服务端建立连接。
   4. 客户端与服务端建立连接成功后，发生打洞消息到服务端。
   5. 服务端转发打洞地址给客户端，客户端发送UDP到用户端打洞地址，成功后走创建好的隧道直连访问内网服务。
   6. 如果打洞没成功，仍可走服务器中转访问。
## 联系我
如需了解更多，请关注微信公众号java-tony：
![java-tony](jrp-doc/images/java-tony.png)
