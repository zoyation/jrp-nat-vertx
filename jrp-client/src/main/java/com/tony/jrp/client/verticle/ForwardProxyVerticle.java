...
//    private static final byte SOCKS5_CMD_BIND = 0x02;
    ...
    public ForwardProxyVerticle(String ipv4, DatagramSocket serverSocket, SocketAddress socketAddress, SecurityService securityService,
                                UserProxy clientProxy) {
    ...
    public void init() {
        ...
        server.connectHandler(netSocket -> {
            ...
            netSocket.closeHandler(voidHandler -> {
                ...
                if (cachedRequest) {
                    ...
                    datagramSocket.send(Buffer.buffer(JRPMsgType.TYPE_LEN + msgId.length())
                            .appendByte(JRPMsgType.CLOSE.getCode())
                            .appendBuffer(msgId), p2pSocketAddress.port(), p2pSocketAddress.host());
                }
            });
        ...
        server.listen(clientProxy.getLocal_port(), res -> {
            if (res.succeeded()) {
                log.info("[{}]内网穿透代理服务启动成功，代理端口：{}。", clientProxy.getType().name(), clientProxy.getLocal_port());
            } else {
                log.error("端口[{}]内网穿透代理服务启动失败：{}",
                        clientProxy.getLocal_port(), res.cause().getMessage(), res.cause());
            }
        });
    }
...
//        options.setIdleTimeout(IDLE_TIMEOUT);
    ...
    private Handler<Buffer> httpAuthAndProtocolHandler(NetSocket socket) {
        ...
        return buffer -> {
            ...
            boolean httpOrHttpsProxy = clientProxy.getType() == ServiceType.HTTP_PROXY || clientProxy.getType() == ServiceType.HTTPS_PROXY || clientProxy.getType() == ServiceType.SMART_PROXY;
            if (httpFlag) {
                ...
                boolean httpsRequest = bufferStr.startsWith(HttpMethod.CONNECT.name());
                boolean getRequest = bufferStr.startsWith(HttpMethod.GET.name());
                //是否为http代理请求
                boolean proxyConnection = bufferStr.contains(PROXY_CONNECTION);
                //是否为http代理服务ip请求
                boolean proxyServerIpRequest = ipv4 != null && bufferStr.contains(ipv4);
                //http://httpbin.org/ip
                ...
                if (proxyServerIpRequest) {
                    log.debug("[{}]为http代理服务ip请求[{}:{}]，直接返回ok.", remoteAddress, ipv4, remotePort);
                ...
                if (proxyConnection) {
                    ...
                    if (httpOrHttpsProxy) {
                        ...
                        //不支持http代理方式，直接关闭
                        log.warn("[{}]不支持代理认证！", remoteAddress);
                        socket.close();
                    }
                    ...
                    //只返回ok，可以客户端配置socks方式访问内网
                    socket.end(Buffer.buffer(securityService.getOKResponse()));
                } else {
                    //不支持socks代理方式，直接关闭
                    log.warn("[{}]不支持的代理方式！", remoteAddress);
                    socket.close();
                }
            } else {
                ...
                if (version == SOCKS4_VERSION && (clientProxy.getType() == ServiceType.SOCKS4 || clientProxy.getType() == ServiceType.SMART_PROXY)) {
                    ...
                    //用户名和host验证，本地运行这儿直接通过
                    String targetHost = socks4a ? domainBuilder.toString() : dstIP;
                    //targetPort转int
                    int targetPortInt = ByteBuffer.wrap(targetPort).order(ByteOrder.BIG_ENDIAN).getShort() & 0xFFFF;
                    log.debug("来自[{}]的SOCKS4代理穿透请求认证通过，目标服务为[{}:{}]！", remoteAddress, targetHost, targetPortInt);
                    ...
                    if (selectedMethod == SOCKS5_AUTH_METHOD_USERNAME_PASSWORD) {
                        // 切换到用户名密码认证处理阶段
                        log.info("socks5切换到用户名密码认证处理阶段：{}", remoteAddress);
    ...
    public void closeUDPSocket(int tcpHashCode) {
        UdpRequest.closeByTcpHashCode(tcpHashCode).forEach(udpHashCode -> {
            ...
            if (cachedRequest != null) {
                ...
                datagramSocket.send(Buffer.buffer(JRPMsgType.TYPE_LEN + cachedRequest.getMsgId().length())
                        .appendByte(JRPMsgType.CLOSE.getCode())
                        .appendBuffer(cachedRequest.getMsgId()), p2pSocketAddress.port(), p2pSocketAddress.host());
            }
        });
    }
    ...
    private boolean validateCredentials(String username, String password) {
        return true; // 这里仅为示例，实际应用中应替换为真实的验证逻辑
    }
            ...
            if (JRPMsgType.CLOSE == msgType) {
                ...
                if (data.length() == 0) {
                    ...
                    switch (proxyProto) {
                        ...
                        case SOCK4A_TCP:
                            // 发送socks4穿透隧道创建成功响应
                            log.debug("SOCK4穿透客户端创建TCP连接成功，发送TCP隧道创建成功响应。");
                        ...
                        case HTTPS:
                            proxyRequest.setTunneled(true);
                            //返回https隧道成功响应
                            clientNetSocket.write(securityService.getHttpsConnectResponse());
                            break;
                    }
                } else {
                    proxyRequest.setTunneled(true);
